...
def test_adsb_single_position_frame_uses_receiver_for_local_cpr() -> None:
    ...
    path = Path(__file__).resolve().parents[2] / "data" / "fixtures" / "adsb" / "adsb1090_multi_8msps.npz"
...
def test_adsb_aircraft_displays_cached_adsbdb_route() -> None:
    ...
    recording = FileIQSource.load(
        Path(__file__).resolve().parents[2] / "data" / "fixtures" / "adsb" / "adsb1090_multi_8msps.npz"
    )
    route = FlightRoute(
        callsign="KLM1023",
        airline_name="KLM",
