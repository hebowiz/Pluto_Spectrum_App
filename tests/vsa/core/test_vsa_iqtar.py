...
def test_committed_rs_iq_tar_sample_is_loadable() -> None:
    path = Path(__file__).resolve().parents[2] / "data" / "fixtures" / "general" / "rs_sample_gfsk_8msps.iq.tar"
...
def test_mhdt4_pattern_failure_falls_back_to_detected_data_psk_sync() -> None:
    recording = FileIQSource.load(
        Path(__file__).resolve().parents[2] / "data" / "fixtures" / "bluetooth" / "hdt" / "bt_mHDT4_capture.iq.tar"
    )
    session = VSASession(
        recording=recording,
