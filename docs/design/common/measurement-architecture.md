...
continuous calibrated IQ
    ├─ Gaussian complex IQ RBW filter → magnitude → software Power Trigger
    ├─ Zero Span TA
...
Power Level TriggerのUIとグラフ線は最終表示と同じ補正済みdBmを使用します。内部detectorはGaussian complex IQ RBW filter後の各sample magnitudeを`iq_full_scale=2048`で正規化したdBFSで動作するため、`dBFS = 設定dBm - (20log10(iq_full_scale) + 固定補正 + 周波数別補正 + 入出力補正)`で変換します。trigger判定と表示は同じRBW特性・振幅校正を共有し、取得record自体には再解析可能なraw IQを保持します。
...
このSnapshot経路は現在TA Free RunのSingle record向けであり、Continuousの欠落を解消するものではありません。Sweep SAはLO pointごとの有限長bufferが連続なら掃引全体を連続転送する必要がなく、主な影響は各pointの転送待ちによるSweep Time増加です。

6 MSPS超の短時間測定には、各RX bufferを独立した連続区間として扱うBuffer Island方式を実装しました。対象は262,144 samples以下のFree Run Continuous、およびPower TriggerのSingle/Continuousです。Free Runは約65,536 samples以上の最小record整数倍（最大4 records）、Power Triggerは262,144 samples以下の最大record整数倍をbuffer長とします。

Power Islandはbuffer取得後にofflineでedgeを走査し、pre/postが同じbuffer内で完成するeventだけを採用します。buffer端の未完成eventは棄却し、境界でtrigger timelineとmeasurement filterをresetします。Auto timeoutはhostの単調時計で管理し、取得済みbuffer内の安全な位置へforced eventを生成します。この方式はbuffer内部recordへの不連続混入を防ぎますが、USB blind time中のeventは観測できません。したがって周期信号や再試行可能なeventには実用性がある一方、希少なone-shot eventの捕捉保証にはPluto側の循環buffer/FPGA triggerが必要です。
