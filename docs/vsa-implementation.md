...
- 16QAMは4回対称の振幅保持detected-data同期を使い、PSK用位相intervalでResult Rangeを切り詰めない。既知Pattern経路は復調symbolを取得した実sample中心を`symbol_time_s`として公開し、Modulation / Symbol Plotが別のfractional位置を再サンプルしてQAM constellationを広げない。QAM Modulation Signal設定はPSK / FSKと独立保存し、Symbol PlotはPSK設定に依存しないPhysical IQ固定とする。Modulation / Symbol PlotのQAM markerは正規化後のI/Q値を表示する。
