...
- 16QAMは4回対称の振幅保持detected-data同期を使い、複数振幅半径が持続するQAM区間を定包絡のFSK/PSK headerや無信号区間から分離する。PSK用位相intervalでResult Rangeを切り詰めない。既知Pattern経路は復調symbolを取得した実sample中心を`symbol_time_s`として公開し、Modulation / Symbol Plotが別のfractional位置を再サンプルしてQAM constellationを広げない。既知symbolはpacket位置と90度carrier ambiguityの確定に使い、CFOは短いPatternだけでなくResult Range全体のQAM decisionから再推定する。QAM Modulation Signal設定はPSK / FSKと独立保存する。Symbol PlotはPSK設定およびQAM Modulation SignalのRaw/Measured選択に依存せず、同期成功時はcarrier補正済みPhysical IQ固定とする。Modulation / Symbol PlotのQAM markerは正規化後のI/Q値を表示する。
