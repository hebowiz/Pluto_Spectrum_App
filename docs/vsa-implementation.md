...
- IQ PowerとInstantaneous Frequencyはcapture全体のdataを保持する。Burst SearchがOFFなら、表示X軸を選択中Result Rangeの前後10%へfitする。Burst SearchがONなら、左端を選択されたburst検出点からResult Range長の10%だけ前、右端をResult Range終了から同じ10%だけ後へfitし、patternやDetected Dataの開始前も確認できるようにする。Pattern Waveformを緑、Result Rangeを青の領域、Pattern/Detected Data Startを縦線で示す。zoom/panでcapture内の他部分も確認できる。
...
- 16QAMは4回対称の振幅保持detected-data同期を使い、複数振幅半径が持続するQAM区間を定包絡のFSK/PSK headerや無信号区間から分離する。PSK用位相intervalでResult Rangeを切り詰めない。既知Pattern経路は粗い整数sample位置の検出後、Result Range全体のQAM decision EVMを使ってfractional symbol timingを再推定し、その実sample中心を`symbol_time_s`として公開する。これによりcapture開始位相に伴うEVM変動と、Modulation / Symbol Plotが別位置を再sampleするconstellation拡大を防ぐ。既知symbolはpacket位置と90度carrier ambiguityの確定に使い、CFOも短いPatternだけでなくResult Range全体のQAM decisionから再推定する。QAM Modulation Signal設定はPSK / FSKと独立保存する。Symbol PlotはPSK設定およびQAM Modulation SignalのRaw/Measured選択に依存せず、同期成功時はcarrier補正済みPhysical IQ固定とする。Modulation / Symbol PlotのQAM markerは正規化後のI/Q値を表示する。
...
- BPSK / QPSK / OQPSK
- pi/4-DQPSK / 8DPSK
- pi/4-QPSK / 8PSK / 16QAM

symbol rate、FSK deviation、TX filter、BT/Alpha相当parameter、mapping名を保持します。
...
2026-08-28にHSTAとの比較で、VSAのTrigger Offset 0ではlevel crossing sampleをrecord先頭にしていたため、最初のburstだけ送信rampまたはpreamble先頭を失い、同じcapture内の後続burstだけ完全になることを確認した。Trigger Offsetが厳密に0のときは16 symbols（capture長未満）のprestoreを共通trigger recorderのring historyから自動保持する。明示的な正または負のTrigger Offsetは従来どおりユーザー指定を優先し、自動prestoreを加算しない。metadataには`acquisition_default_prestore_samples`を記録する。

同日の実機再試験で、同期`capture_iq_block()`をrecord長単位で繰り返す待受には、block返却後のpower判定から次のRX buffer再投入まで受信blind timeが残ることが判明した。10 packet列の途中から取得開始する、先頭または末尾packetが欠ける、取得ごとに検出event数が変わる原因になる。Acquisition I/Q Power Triggerは65,536-sample RX producerを先行起動し、共通`IQStreamBuffer`を独立cursorで継続消費する方式へ変更した。trigger判定とrecord組立はproducerを停止せず行い、block境界をまたぐpre/post-trigger IQも共通sample timelineで連結する。consumer overrunは欠落recordとして黙認せず明示エラーにする。
